...
def query():
	result = [link for link in links if link.id == 15]
	if len(result): 
		return result[0].votes
		
def unit_5_query():
	assert query() == 1244
	print 'unit_5_query all tests passed'
...
def query():
	result = filter(lambda link: link.submitter_id == 62443, links)
	result.sort(key = lambda x: x.submitted_time)
	return result

def unit_6_querying(debug = False):
	assert len(query()) == 3
	if debug:
		for link in query():
			print '{0}'.format(link.submitted_time)
	print 'unit_6_querying all tests passed'
	
unit_6_querying()

# type of databases
#
# Relational Database			Google App Engine Data Store
# SQL : Postgres-sql			
#		MySQL
#		SQLite					Amazon Dyanmo
#		Oracle					
#
#								NoSQL
#									Mongo
...
"""
	...
	(fetch)		all column 	table				constraint
"""
...
def unit_10_databases_in_python():
	assert query() == 81
	print 'unit_10_databases_in_python all tests passed'

unit_10_databases_in_python()
...
def unit_12_advanced_statement():
	assert query() == 15
	print 'unit_12_advanced_statement all tests passed'

unit_12_advanced_statement()
...
def query():
	...
	return [Link(*x).id for x in c.fetchall()]
	
def unit_13_order_by():
	assert query() == [15,18,101]
	print 'unit_13_order_by all tests passed'
...
def unit_16_query_links():
	assert link_by_id(23) == Link(23, 62945, 1333894106.0, 351,
	         "Learn Haskell Fast and Hard",
	         "http://yannesposito.com/Scratch/en/blog/Haskell-the-Hard-Way/")
	print 'unit_16_query_links all tests passed'
...
# indexes decrease the speed of database insert


# another way to do index
# tree
...
# ACID
# A - atomicity		all parts of a transaction succeed or fail together
# C - consistency	the database will always be consistent
# I - isolation		no transaction can interfere with another
# D - durability	once the transaction is committed, it wont be lost

# Google App Engine Data Store
#
# entity
# - columns are not fixed
# - all have an ID (manual specify or system assign)
# - parent / ancestors
#
# GQL
# - all query begin with SELECT *
# - no joins
# - all queries must be indexed (cant be too slow)
# - by default google app engine will do auto index on columns
#
# Datastore
# - sharded
# 		probably wont have to think about scaling
# - replicated
# 		query will be quick
#		will have to think about consistency

# Google Datastore Types
# [int, float, string, date, time, datetime, email, link, postal, address... text]
