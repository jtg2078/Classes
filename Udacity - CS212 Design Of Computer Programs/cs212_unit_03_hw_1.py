...
def parse(start_symbol, text, grammar):
    ...
    def parse_atom(atom, text):
        #print 'atom: {0}  text: {1}'.format(atom, text)
        if atom in grammar:  # Non-Terminal: tuple of alternatives
            ...
            #print 'regex: {0}  text: {1}'.format(tokenizer % atom, text)
...
JSON = grammar("""value => string | number | array | object
string => \"[^\"]*\"
...
object => \{ members \} | \{\}
pair => string \: value
members => pair , members | pair""", whitespace='\s*')
...
print test()
#print json_parse('{"occupation":"rides the rodeo"}')
#print json_parse('{"state": "CO"}')
#print json_parse('{"age": 21}')
#print json_parse('"rides the rodeo"')
#z = re.match(r"\s*(\[)", '["testing", 1, 2, 3]')
#print z.span()
