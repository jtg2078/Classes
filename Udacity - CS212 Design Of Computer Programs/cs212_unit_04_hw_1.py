...
def bsuccessors3(state):
	...
	if light == 0:
		return dict(((here - frozenset([p1, p2]), there | frozenset([p1, p2]), 1), (set([p1, p2]), '->')) for p1 in here for p2 in here)
	else:
		return dict(((here | frozenset([p1, p2]), there - frozenset([p1, p2]), 0), (set([p1, p2]), '<-')) for p1 in there for p2 in there)
