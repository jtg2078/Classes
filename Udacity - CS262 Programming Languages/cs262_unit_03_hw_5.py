...
# basically this function return all the terminal node in the parse tree
def traverse(grammar, start):
	isTerminal = True
	u = []
	for name, rule in grammar:
		if name == start:
			isTerminal = False
			for n in rule:
				u =  u + traverse(grammar, n)
	if isTerminal == True:
		return [start]
	else:
		return u 

# so this method function called traverse function
# to get a list of terminal nodes and iterate through it to see if the
# passed-in utterance occured more than once
# it worked, but the test of utterance with length of 1(single element)
# failed, so i just added the extra check condition, i guess with just
# one element, you cannot have ambiguity?
# (i guess i'll find out the right way to solve this problem when the
#  answer is posted) 
def isambig(grammar, start, utterance):
	output = traverse(grammar, start)
	duplicate = 0
	for i in range(len(output)):
		if output[i] == utterance[0]:
			# could be a match
			if i + len(utterance) <= len(output):
				# length check is good
				a = output[i:i + len(utterance)]
				if a ==  utterance:
					duplicate += 1
					if duplicate >= 2 and len(utterance ) > 1:
						return True
	return False
